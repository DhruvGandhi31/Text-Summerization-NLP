...
An implementation of text summarization using the Pegasus model with the Transformers library.
...
1. Loading Data and Preprocessing: It downloads a dataset and preprocesses it for training the model.
...
5. Saving Model and Tokenizer: Once training and evaluation are complete, it saves the trained model and tokenizer for future use.
...
## Deployment
This model is deployed on AWS using Amazon EC2 and Elastic Container Registry(ECR) services. The application is containerized using Docker and integrated with GitHub Actions for continuous integration and deployment. FastAPI is used to create a simple and efficient interface for interacting with the model.
